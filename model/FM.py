...
# def train_iter(model_MF, optimizer, data_loder, criterion):
#     model_MF.train()
#     total_loss = 0
#     total_len = 0
#
...
#
#
# def val_iter(model_MF, data_loader):
#     model_MF.eval()
...
#
#     # 模型初始化
#     model_MF = FM(u_max_id, i_max_id, id_embedding_dim).to(device)
#     optimizer = torch.optim.Adam(model_MF.parameters(), lr=learning_rate, weight_decay=weight_decay)
#     loss_func = torch.nn.MSELoss().to(device)
#
#     # 训练模型
#     best_val_mse, best_val_epoch = 10, 0
#     for epoch in range(epochs):
#         loss = train_iter(model_MF, optimizer, train_loader, loss_func)
#         mse = val_iter(model_MF, val_loader)
#         print("epoch:{}, loss:{:.5}, mse:{:.5}".format(epoch, loss, mse))
#         if best_val_mse > mse:
#             best_val_mse, best_val_epoch = mse, epoch
#             torch.save(model_MF, 'best_model')
#     print("best val epoch is {}, mse is {}".format(best_val_epoch, best_val_mse))
#     model_MF = torch.load('best_model').to(device)
#     test_mse = val_iter(model_MF, test_loader)
