...
def generate_controller_state(value_s, value_t, value_g):
    # if norm == 'Y':
    #     user = max_norm(user)
    #     pos = max_norm(pos)
    #     neg = max_norm(neg)
    ...
    return torch.cat((
        ...
        value_s - value_t, value_t - value_g, value_g - value_s
    ), dim=1)
...
def train(args):
    ...
    controller = Controller(6, args.device).to(args.device)

    # optimizer = torch.optim.Adam(params=list(model.parameters()).append(list(controller.parameters())), lr=args.lr,
    ...
    for epoch in range(args.n_epochs):
        ...
        for x_u, x_i, y, y_t in train_dataloader:
            ...
            value_g = y_t.reshape(-1, 1)

            state = generate_controller_state(value_s, value_t, value_g)
            trade_off = controller(state).squeeze()
            loss = (1 - trade_off) * loss_func(torch.sigmoid(y_pre), y) + \
