...
import numpy as np
from model.Wide_Deep_BCE import Wide_Deep, Controller
...
def train(args):
    ...
    model_teacher = Wide_Deep(feat_sizes, args.dim, linear_feature_columns, dnn_feature_columns, [64, 32]).to(
        args.device)
    saved_teacher_path = './saved/{}/model_{}/teacher_dim{}.pt'.format(args.data_name, args.model_name, args.dim)
    model_teacher.load_state_dict(torch.load(saved_teacher_path))
    true_label = []
    teacher_logits = []
    for x, y in train_dataloader_raw:
        x, y = x.to(args.device).float(), y.to(args.device).float().reshape(-1, 1)
    ...
    saved_path = './saved/{}/model_{}/student_dim{}_state{}.pt'.format(args.data_name, args.model_name, args.dim,
                                                                       args.state)
    ...
    for epoch in range(args.n_epochs):
        ...
        for x, y_g, y_t in train_dataloader:
            x, y_g, y_t = x.to(args.device).float(), y_g.to(args.device).float().reshape(-1, 1), y_t.to(
                args.device).float().reshape(-1, 1)
            y_pre = model(x)

            value_s = torch.sigmoid(y_pre / args.temp)
            value_t = torch.sigmoid(y_t / args.temp)
            value_g = y_g
            value_o = value_t
