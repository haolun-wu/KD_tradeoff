...
import numpy as np
from model.MF import model_MF, Controller
...
def generate_controller_state(value_s, value_t, value_g, state_type):
    if state_type == 1:
        return torch.cat((
...
def train(args):
    ...
    controller_optimizer = torch.optim.Adam(controller.parameters(),
                                            lr=args.lr,
                                            weight_decay=0)
    # single_optimizer = torch.optim.Adam(list(model.parameters()) + list(controller.parameters()),
    #                                     lr=args.lr,
    #                                     weight_decay=args.wd)
    loss_func = torch.nn.BCELoss(reduction='none').to(args.device)
    ...
    for epoch in range(args.n_epochs):
        ...
        for x_u, x_i, y_g, y_t in train_dataloader:
            ...
            value_s = torch.sigmoid(y_pre / args.temp).reshape(-1, 1)
            value_t = torch.sigmoid(y_t / args.temp).reshape(-1, 1)
            value_g = y_g.reshape(-1, 1)

            state = generate_controller_state(value_s, value_t, value_g, args.state_type)
            trade_off = controller(state).squeeze()
            loss = (1 - trade_off) * loss_func(torch.sigmoid(y_pre), y_g) + \
                   trade_off * args.temp ** 2 * loss_func(torch.sigmoid(y_pre / args.temp),
...
if __name__ == '__main__':
    ...
    state_list = [1, 2, 3]
    result_table = PrettyTable(['Dataset', 'state', 'ACC_train', 'ACC_test'])

    for state_type in state_list:
        args.state_type = state_type
        train_accuracy, test_accuracy = train(args)
        result_table.add_row([args.data_name, state_type, train_accuracy, test_accuracy])
        print("------------------------------")
